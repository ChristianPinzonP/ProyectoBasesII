...
CREATE OR REPLACE PROCEDURE GENERAR_ESTADISTICAS_ESTUDIANTE (
    ...
    v_minima NUMBER;
BEGIN
    -- Obtener estad�sticas desde la vista
    SELECT 
    ...
    -- Mostrar en consola
    DBMS_OUTPUT.PUT_LINE('Total ex�menes: ' || v_total_examenes);
    ...
    DBMS_OUTPUT.PUT_LINE('M�xima: ' || v_maxima);
    DBMS_OUTPUT.PUT_LINE('M�nima: ' || v_minima);
...
CREATE OR REPLACE PACKAGE PKG_PREGUNTA AS
  ...
  -- NUEVO: Procedimiento para obtener preguntas visibles para un docente espec�fico
...
CREATE OR REPLACE PACKAGE BODY PKG_PREGUNTA AS
    ...
    BEGIN
        ...
        IF SQL%ROWCOUNT = 0 THEN
            RAISE_APPLICATION_ERROR(-20001, 'No se encontr� la pregunta con ID: ' || p_id_pregunta);
        END IF;
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('M�xima: ' || v_max_nota);
    DBMS_OUTPUT.PUT_LINE('M�nima: ' || v_min_nota);
